...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Paathu voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Paathu') && conf.TALKING_Paathu !== 'true') {        
        ...
        var finm = message.message.replace('Paathu', '').replace(' ', '')   
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_Paathu == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'T fonksiyonel Paathu özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Paathu yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Paathu yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Paathu Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Paathu Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Paathu features. Turn your account into a ai chatbot!'
    already_on = 'Paathu artificial intelligence is already fully functional.'
    already_off = 'Paathu artificial intelligence is currently running semi-functional.'
    succ_on = 'Paathu Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Paathu Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Paathu സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Paathu കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Paathu AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Paathu പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Paathu സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'eva ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.eva on / off' }, (async (message, match) => {
    var Paathu_status = `${conf.TALKING_Paathu}`
    if (match[1] == 'on') {
        if (Paathu_status == 'true') {
