...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('bot') && conf.FULLEVA !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/wechat?bid=159723&key=CmLHIhtAFYa7WnQM=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/wechat?bid=159723&key=CmLHIhtAFYa7WnQM=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel paathu özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
